...
class TimeInvariantUNet3D(nn.Module):
    """
    UNet3D model without time-axis pooling
    """
    def __init__(self, in_channels, out_channels, final_sigmoid, f_maps=64, layer_order='cr', num_groups=8,
                 **kwargs):
        super(TimeInvariantUNet3D, self).__init__()
        ...
        for i, out_feature_num in enumerate(f_maps):
            if i == 0:
                ...
                encoder = Encoder(f_maps[i - 1], out_feature_num, basic_module=DoubleConv, pool_kernel_size=(1,2,2),
                                  conv_layer_order=layer_order, num_groups=num_groups)
        ...
        for i in range(len(reversed_f_maps) - 1):
            ...
            decoder = Decoder(in_feature_num, out_feature_num, basic_module=DoubleConv, scale_factor=(1,2,2),
                              conv_layer_order=layer_order, num_groups=num_groups)
